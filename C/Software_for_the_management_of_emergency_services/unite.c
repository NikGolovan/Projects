...
void declancherAlerte(Alerte **alerte, Unites **unite, int *iCompteurAlerte, int *iCompteurUnite) {
  int i = 0;
  int k = 0;
  int iCodeDonneAlerte = 0; /* code d'alerte saisi par l'utilisateur */
  int iCodeDonneUnite = 0; /* code d'unité saisi par l'utilisateur */
  int iTmpPositionAlerte = 0; /* sert à parcourir les cases du tableau Alerte */
  int iTmpPositionUnite = 0; /* sert à parcourir les cases du tableau Unites */
  int iBoucleCompteurAlerte = 0; /* compteur pour les boucles d'Alertes */
  int iBoucleCompteurUnite = 0; /* compteur pour les boucles d'Unites */
  int iUnitesEnReposPresents = 0; /* compteur qui dit si il y a les unités en repos */
  int iChoix = 0; /* saisie de l'utilisateur pour le cas de force majeure */
  int iNbUniteDonne = /* contient la saisie de quantité d'unité pour le traitement d'une alerte */
  ...
  if(iBoucleCompteurUnite == 0 && iUnitesEnReposPresents == 0) {
    printf("Il n'y a aucune unité en repos.\n");
    return;
  }
  ...
  printf("Donnez le nombre d'unité à envoyer : \n");
  scanf("%d%*c", &iNbUniteDonne);

  (*alerte)[iTmpPositionAlerte].iCodeUniteQuiTraite = malloc(iNbUniteDonne * sizeof(int));

  for (k = 0; k < iNbUniteDonne; k++) {
    ...
    (*alerte)[iTmpPositionAlerte].iNbUniteQuiTraite = (*alerte)[iTmpPositionAlerte].iNbUniteQuiTraite + 1;
    (*alerte)[iTmpPositionAlerte].iCodeUniteQuiTraite[k] = iCodeDonneUnite;
